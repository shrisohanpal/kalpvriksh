...
import {
  ...
  ORDER_LIST_MY_FAIL,
  ORDER_LIST_FAIL,
  ORDER_LIST_BY_VENDOR_REQUEST,
  ORDER_LIST_BY_VENDOR_SUCCESS,
  ORDER_LIST_BY_VENDOR_FAIL,
  ORDER_LIST_SUCCESS,
  ORDER_LIST_REQUEST,
  ORDER_DISPATCH_FAIL,
...
export const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    order.orderItems.map(async (orderItem) => {
      await axios.post(`${baseUrl}/api/orders`, { ...order, orderItems: [orderItem] }, config)
    })
...
export const dispatchOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `${baseUrl}/api/orders/${order._id}/dispatch`,
      {},
      config
    )
...
export const listOrdersByVendor = (vendorId) => async (dispatch, getState) => {
  try {
    dispatch({
      type: ORDER_LIST_BY_VENDOR_REQUEST
    })
    ...
    const { data } = await axios.get(`${baseUrl}/api/orders/byvendor?vendorId=${vendorId}`, config)

    dispatch({
      type: ORDER_LIST_BY_VENDOR_SUCCESS,
      payload: data,
    })
  } catch (error) {
    const message =
    ...
    dispatch({
      type: ORDER_LIST_BY_VENDOR_FAIL,
      payload: message,
    })
  }
}
